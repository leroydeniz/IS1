...
//	private JButton jButtonClose = new JButton(ResourceBundle.getBundle("Etiquetas").getString("Close"));
	...
	private final JLabel lblCrearApuesta = new JLabel(ResourceBundle.getBundle("Etiquetas").getString("ApostarGUI.lblCrearApuesta.text")); //$NON-NLS-1$ //$NON-NLS-2$

	public ApostarGUI(Vector<domain.Event> v, Cliente pcli, Event pevento) {
		try {
			currentClient=pcli;
			pevent = pevento;
			jbInit(v);
	...
	private void jbInit(Vector<domain.Event> v) throws Exception {
		...
		jComboBoxEvents.setBounds(460, 66, 250, 20);
		jComboBoxEvents.setModel(modelEvents);
		jComboBoxEvents.setEnabled(false);
		jComboBoxQuestions.setBounds(455, 109, 250, 20);
		...
		jCalendar.setBounds(40, 70, 225, 150);

		this.getContentPane().add(jCalendar);
		jLabelEventDate.setBounds(41, 40, 140, 25);
		getContentPane().add(jLabelEventDate);
		lblSeleccionarPresgunta.setBounds(291, 108, 155, 20);
		...
		jComboBoxAnswers.setBounds(460, 152, 250, 20);
		...
		lblSeleccionarRespuesta.setBounds(291, 151, 155, 20);
		getContentPane().add(lblSeleccionarRespuesta);
		lblSeleccionarEvento.setBounds(291, 65, 155, 20);
		...
		lblCrearApuesta.setHorizontalAlignment(SwingConstants.CENTER);
		lblCrearApuesta.setFont(new Font("Lucida Grande", Font.PLAIN, 16));
		lblCrearApuesta.setBounds(250, 6, 249, 26);
		
		getContentPane().add(lblCrearApuesta);
...
//					public void propertyChange(PropertyChangeEvent propertychangeevent) {
				if (propertychangeevent.getPropertyName().equals("locale")) {
					...
					try {
						...
						if (pevent != null) {
							jComboBoxEvents.setSelectedIndex(index);			
							pevent = null;
						}
...
//			    		  Component o=(Component) jCalendar.getDayChooser().getDayPanel().getComponent(i+offset);; 
	...
	private void jButtonCreate_actionPerformed(ActionEvent e) {
		...
		try {
			...
			int mensaje = facade.createApuesta(selectedAnswer, currentClient, montoIngresadoFloat);
